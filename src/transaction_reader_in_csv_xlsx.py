...
def transaction_reader_in_csv(file_path: Path) -> list[dict]:
    """Функция принимает путь до csv-файла в качестве аргумента и выдает список словарей с данными о финансовых
    транзакциях.
...
def transaction_reader_in_xlsx(file_path: Path) -> list[dict] | str:
    """Функция принимает путь до xlsx-файла в качестве аргумента и выдает список словарей с данными о финансовых
    транзакциях.
    Если файл пустой, содержит не список или не найден, функция возвращает пустой список.
